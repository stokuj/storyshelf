...
export const actions: Actions = {
	...
	email: async ({ request, fetch }) => {
		...
		const res = await fetch(`${serverApiBase()}/users/me/email/`, {
			method: 'PATCH',
			headers: { 'Content-Type': 'application/json' },
	...
	password: async ({ request, fetch }) => {
		...
		const res = await fetch(`${serverApiBase()}/users/me/password/`, {
			method: 'PATCH',
			headers: { 'Content-Type': 'application/json' },
	...
	avatar: async ({ request, fetch }) => {
		...
		const res = await fetch(`${serverApiBase()}/users/me/avatar/`, {
			method: 'PATCH',
			body: data,
			credentials: 'include'
		});
